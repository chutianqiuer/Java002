...
public class OrderService extends ServiceImpl<OrderMapper, Order> {
    ...
    public Order createOrder(Order order) {
        ...
        // Deduct stock before saving order (Seata AT mode handles rollback automatically)
        ...
        // TEST MODE: If remark is FORCE_FAIL_AFTER_DEDUCT, throw exception to simulate failure
        // This tests that Seata AT mode will rollback the stock deduction
        if ("FORCE_FAIL_AFTER_DEDUCT".equals(order.getRemark())) {
            throw new RuntimeException("TEST_MODE: Simulated order save failure for Seata rollback testing");
        }
        ...
        // Save order - Seata will rollback if this fails
        this.save(order);

        // Send OrderCreatedEvent after order saved successfully (outside transaction)
