...
# 1. Check RocketMQ containers are running
echo "=== 1. 检查 RocketMQ 容器状态 ==="
...
# 2. Verify broker registered to namesrv using mqadmin
echo "=== 2. 验证 broker 注册到 namesrv ==="
CLUSTER_INFO=$(docker exec mall-rocketmq-broker sh mqadmin clusterList -n rocketmq-namesrv:9876 2>&1)
echo "$CLUSTER_INFO"

if echo "$CLUSTER_INFO" | grep -q "DefaultCluster"; then
  echo "✓ Broker registered to namesrv successfully"
else
  echo "ERROR: Broker not registered to namesrv"
  echo "$CLUSTER_INFO"
  exit 1
fi
echo ""

# 3. Check if topic exists, create if not
echo "=== 3. 检查 topic 是否存在 ==="
TOPIC_INFO=$(docker exec mall-rocketmq-broker sh mqadmin topicList -n rocketmq-namesrv:9876 2>&1)
echo "$TOPIC_INFO" | head -10

if echo "$TOPIC_INFO" | grep -q "order-created-topic"; then
  echo "✓ Topic order-created-topic exists"
else
  echo "Topic not found, creating..."
  docker exec mall-rocketmq-broker sh mqadmin updateTopic -n rocketmq-namesrv:9876 -t order-created-topic -c DefaultCluster -r 4 -w 4 2>&1
  echo "✓ Topic order-created-topic created"
fi
echo ""
...
# 6. Wait for consumer to process
echo "=== 6. 等待 5 秒让消费者处理 ==="
sleep 5
echo ""

# 7. Check operation_logs in mall_admin database
echo "=== 7. 查询 mall_admin.operation_logs ==="
...
# 8. Verify ORDER_NO in logs
if [ -n "$ORDER_NO" ]; then
  echo "=== 8. 验证 ORDER_NO 一致性 ==="
  ...
  if [ "$LOG_ORDER_NO" -gt 0 ]; then
    ...
    echo ""
    echo "=== Last 120 lines of mall-order log ==="
    tail -120 /tmp/mall-logs/mall-order.log 2>/dev/null || true
    echo ""
    echo "=== Last 120 lines of mall-admin log ==="
    tail -120 /tmp/mall-logs/mall-admin.log 2>/dev/null || true
    echo ""
    echo "=== Last 120 lines of broker log ==="
    docker logs mall-rocketmq-broker --tail 120 2>&1 || true
    exit 1
  fi
fi
